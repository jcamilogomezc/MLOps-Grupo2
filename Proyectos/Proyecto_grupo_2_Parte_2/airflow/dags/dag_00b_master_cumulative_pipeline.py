"""
Master DAG: Complete ML Pipeline with Cumulative Batch Training
This DAG orchestrates the entire ML pipeline with parallel cumulative batch training.

Pipeline Flow:
1. Download diabetes dataset
2. Split into train/val/test (70/15/15)
3. Store training data in PostgreSQL batches of 15,000 records
4. Create cumulative database views dynamically based on actual data size
5. Generate training configurations based on cumulative views
6. Train models on all cumulative datasets in parallel (N models per dataset)
7. Select best model across all cumulative trainings
8. Publish best model to production

Key Innovation: Dynamic parallel cumulative training
- Cumulative datasets generated automatically based on actual data size
- 4 model types per dataset (Logistic Regression, Decision Tree, Random Forest, Gradient Boosting)
- Total models trained = (number of batches) × 4 model types
- Finds optimal data size and model type combination
"""
...
# ===================================================================
"""
DYNAMIC CUMULATIVE BATCH TRAINING PIPELINE SUMMARY
...
Dataset → Train (70%) / Val (15%) / Test (15%)
             ↓
        PostgreSQL Storage
        (N batch tables + cumulative views)
             ↓
    Generate Training Configs
    (Based on actual data size)
             ↓
    ╔════════════════════════════════╗
    ║ PARALLEL CUMULATIVE TRAINING   ║
    ║ (Dynamically generated)        ║
    ╠════════════════════════════════╣
    ║ Cumulative 0: batch_size       ║  → 4 models
    ║ Cumulative 1: batch_size × 2   ║  → 4 models
    ║ Cumulative 2: batch_size × 3   ║  → 4 models
    ║ ...                            ║
    ║ Cumulative N: all data         ║  → 4 models
    ╚════════════════════════════════╝
             ↓
       N × 4 Trained Models
             ↓
    ...
    Evaluate on Full Validation
             ↓
    Deploy to Production (MLflow)

Models Trained per Cumulative Dataset:
---------------------------------------
...
Total Training Combinations:
- Dynamically determined = (num_batches) × 4 models
- Example: 7 batches × 4 models = 28 models

Key Features:
--------------
- Batch size: 15,000 records (configurable in dag_03b)
- Number of batches: Calculated automatically from dataset size
- Cumulative views: Created dynamically for each batch combination
- Training configs: Generated at runtime based on actual data
...
- Precision
- Recall
- F1 Score (primary selection criterion)
- ROC-AUC (secondary criterion)

Expected Insights:
------------------
- Optimal training data size for best performance
- Best model type for diabetes readmission prediction
- Performance scaling with increasing data size
- Efficiency vs accuracy trade-offs

Execution Time:
---------------
- Sequential: ~3-4 hours (depends on data size)
- With parallel training: ~1-1.5 hours (estimated)
"""
